...
function getInnerDiameterMM(pt, diameter, std) {
    ...
    const stdKey = (std && specsForD[std]) ? std : Object.keys(specsForD)[0];
    const s = specsForD[stdKey];
    if (!s) return null;
...
function standardsForTypeAndDiameter(pt, diameter) {
    const specsForD = pipeSpecs[pt] && pipeSpecs[pt].specs[String(diameter)];
    return specsForD ? Object.keys(specsForD) : [];
}
...
function buildFlowCalculator() {
    ...
    toolModalBody.innerHTML = `
        ...
        <div class="tool-row">
            ...
            <div class="tool-form-group">
                <label>Pressure / Standard (PN)</label>
                <select id="flowStandard"></select>
            </div>
        </div>
        <div class="tool-form-group">
        ...
        <div class="tool-note">Calculation based on the internal diameter for the selected pipe size AND pressure rating (SDR/thickness), so results reflect the actual wall thickness. Flow rate Q = A × V, where A is the internal cross-sectional area.</div>
    `;
    const typeSel = document.getElementById('flowPipeType');
    const diaSel  = document.getElementById('flowDiameter');
    const stdSel  = document.getElementById('flowStandard');
    function fillStandards() {
        const stds = standardsForTypeAndDiameter(typeSel.value, diaSel.value);
        stdSel.innerHTML = stds.map(s => `<option value="${s}">${s}</option>`).join('');
    }
    ...
    diaSel.addEventListener('change', fillStandards);
    fillDiameters();
    document.getElementById('flowCalcBtn').addEventListener('click', () => {
        ...
        if (!d || !std || !v || v <= 0) { box.classList.remove('show'); showErrorMessage('Please select diameter, pressure rating, and enter a valid velocity.'); return; }
        const idMM = getInnerDiameterMM(pt, d, std);
...
function buildPressureLoss() {
    ...
    toolModalBody.innerHTML = `
        ...
        <div class="tool-note">Estimated using the Hazen-Williams formula (C = 150, typical for new plastic pipe), using the internal diameter of the selected pressure rating (SDR/thickness). Results are indicative — for final hydraulic design, consult full project calculations.</div>
    `;
    const typeSel = document.getElementById('plPipeType');
    const diaSel = document.getElementById('plDiameter');
    const stdSel = document.getElementById('plStandard');
    function fillStandards() {
        const stds = standardsForTypeAndDiameter(typeSel.value, diaSel.value);
        stdSel.innerHTML = stds.map(s => `<option value="${s}">${s}</option>`).join('');
    }
    ...
    document.getElementById('plCalcBtn').addEventListener('click', () => {
        ...
        if (!d || !std || !qLs || qLs <= 0 || !length || length <= 0) { showErrorMessage('Please select diameter, pressure rating, and enter valid flow and length values.'); return; }
        const idMM = getInnerDiameterMM(pt, d, std);
        if (!idMM) { showErrorMessage('No data available for this size.'); return; }
        const idM = idMM / 1000;
        const qM3s = qLs / 1000;
